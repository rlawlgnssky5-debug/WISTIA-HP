(function(global){
 ...
 function initArCdRatio(selector='#arRatioExperience'){
  ...
  const listeners=[],urls=new Map(),pending=new Map();let value=70,sourceRatio=70,switchToken=0,resume=false,position=0,destroyed=false,dragging=false,dragStart=0,dragValue=70,dragMoved=0
  ...
  function togglePlay(){error.hidden=true;touched();if(audio.paused)audio.play().catch(()=>{error.hidden=false;playing(false)});else audio.pause()}
  ...
  function setValue(next){touched();const selected=clamp(next);if(selected===value)return;value=selected;audio.pause();resume=false;render();switchSource(nearest(value),false)}
  ...
  on(volume,'click',()=>{audio.muted=!audio.muted;volume.classList.toggle('is-muted',audio.muted);volume.setAttribute('aria-pressed',String(audio.muted));volume.setAttribute('aria-label',audio.muted?'음소거 해제':'음소거')})

...
function arComparisonSection(){return '<section class="ar-compare section" aria-labelledby="arCompareTitle" data-ar-compare><div class="shell"><h2 id="arCompareTitle" class="ar-compare-title"><span class="ar-compare-title-1">같은 축가,</span><br><strong class="ar-compare-title-2">다른 준비.</strong></h2><div class="ar-compare-table" role="table" aria-label="일반 제작과 WISTIA 비교"><div class="ar-compare-wistia-panel" aria-hidden="true"></div><div class="ar-compare-head" role="row"><span>비교 기준</span><span>일반 제작</span><span>WISTIA</span></div><div class="ar-compare-rows">'+AR_COMPARE_ROWS.map((row,i)=>'<div class="ar-compare-row" role="row" data-ar-compare-row style="--row-i:'+i+'"><div class="ar-compare-cell ar-compare-criteria" tabindex="0"><span class="ar-compare-index">'+String(i+1).padStart(2,"0")+'</span><strong>'+row[0]+'</strong></div><div class="ar-compare-cell ar-compare-general">'+row[1]+'</div><div class="ar-compare-cell ar-compare-wistia"><span class="ar-compare-arrow" aria-hidden="true">→</span><strong>'+row[2]+'<span class="ar-compare-wistia-underline" aria-hidden="true"></span></strong></div></div>').join("")+'</div></div><p class="ar-compare-note">제공 범위는 업체와 상품에 따라 달라질 수 있습니다.</p></div></section>'}
let arCompareObserver=null
function initArCompare(){arCompareObserver?.disconnect();const section=document.querySelector("[data-ar-compare]");if(!section)return;const rows=[...section.querySelectorAll("[data-ar-compare-row]")];const clearFocus=()=>rows.forEach(r=>{r.classList.remove("is-focused","is-dimmed")});const focusRow=target=>{rows.forEach(r=>{const active=r===target;r.classList.toggle("is-focused",active);r.classList.toggle("is-dimmed",!active)})};rows.forEach(row=>{row.addEventListener("mouseenter",()=>focusRow(row));row.addEventListener("focusin",()=>focusRow(row));row.addEventListener("click",()=>{if(row.classList.contains("is-focused"))clearFocus();else focusRow(row)})});const table=section.querySelector(".ar-compare-table");table.addEventListener("mouseleave",()=>{if(!table.matches(":focus-within"))clearFocus()});table.addEventListener("focusout",e=>{if(!table.contains(e.relatedTarget)&&!table.matches(":hover"))clearFocus()});document.addEventListener("click",e=>{if(!table.contains(e.target))clearFocus()});if(matchMedia("(prefers-reduced-motion: reduce)").matches){section.classList.add("is-visible");return}arCompareObserver=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting){section.classList.add("is-visible");arCompareObserver.disconnect()}}),{threshold:.2,rootMargin:"0px 0px -8% 0px"});arCompareObserver.observe(section)}

...
function setSoloReview(index){const root=document.querySelector("[data-review-showcase]");if(!root)return;const slides=[...root.querySelectorAll("[data-review-slide]")],total=slides.length;if(!total)return;soloReviewIndex=(index+total)%total;slides.forEach((slide,i)=>{const active=i===soloReviewIndex,wasActive=slide.classList.contains("is-active");slide.classList.toggle("is-leaving",wasActive&&!active);slide.classList.toggle("is-active",active);if(active)slide.classList.remove("is-leaving");slide.setAttribute("aria-hidden",String(!active))});const activeSlide=slides[soloReviewIndex],screen=root.querySelector(".solo-review-phone-screen");if(screen)screen.style.setProperty("--screen-fill",activeSlide.dataset.screenFill||"#050505");const status=root.querySelector("[data-review-status]");if(status)status.textContent=String(soloReviewIndex+1).padStart(2,"0")}
function startSoloReviewAuto(){stopSoloReviewAuto();const root=document.querySelector("[data-review-showcase]");if(!root||matchMedia("(prefers-reduced-motion: reduce)").matches)return;soloReviewTimer=setInterval(()=>{if(!root.matches(":hover")&&!root.matches(":focus-within"))setSoloReview(soloReviewIndex+1)},5600)}

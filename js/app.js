...
function priceBar(key){return '<div class="fixed-price consult-bar" hidden><div>'+consultButton(key)+'</div></div>'}
...
function reviews(){return '<section class="shell section reviews-section" id="reviews">'+heading('','직접 전해주신 이야기','위스티아 이용 고객의 실제 후기입니다')+'<div class="review-excerpts">'+REVIEW_QUOTES.slice(0,2).map(q=>'<blockquote><p>“'+escapeHtml(q.quote)+' '+escapeHtml(q.body)+'”</p><button class="text-link" data-review-open="'+q.index+'">후기 원문 보기 '+arrow()+'</button></blockquote>').join('')+'</div><details class="review-more"><summary>고객 후기 원문 11개 보기</summary><div class="review-original-list">'+ACTUAL_REVIEW_IMAGES.map((src,i)=>'<button data-review-open="'+i+'" aria-label="후기 '+(i+1)+' 원문 확대">'+img(src,'고객 후기 '+(i+1))+'</button>').join('')+'</div></details></section>'}
function openReview(index){index=Math.max(0,Math.min(ACTUAL_REVIEW_IMAGES.length-1,index));showDialog('<h2>고객 후기 원문</h2><div class="review-paging"><button data-review-open="'+((index+10)%11)+'" aria-label="이전 후기">←</button><span>'+(index+1)+' / 11</span><button data-review-open="'+((index+1)%11)+'" aria-label="다음 후기">→</button></div><div class="review-original">'+img(ACTUAL_REVIEW_IMAGES[index],'고객이 보내주신 후기 원문',true)+'</div>','review-dialog')}

function arHookHero(p){return '<section class="ar-hook-hero" aria-labelledby="arHookTitle"><div class="ar-hook-media">'+img("assets/img/ar-detail/solo-live-proof.jpg","실제 고객의 본식 AR 축가 현장",true).replace('<img ','<img class="ar-hook-fallback" ')+'<video autoplay muted loop playsinline preload="auto" poster="assets/img/ar-detail/solo-live-proof.jpg" aria-label="실제 고객의 본식 AR 축가 현장 영상"><source src="'+escapeHtml(p.resultVideo)+'" type="video/mp4">영상을 재생할 수 없는 브라우저입니다</video><span class="ar-hook-video-source">실제 고객님이 보내주신 본식 영상</span><button type="button" class="ar-hook-video-control" data-ar-video-toggle aria-label="실제 고객 본식 영상 재생"><span aria-hidden="true">▶</span><b>영상 재생</b></button></div><div class="ar-hook-copy shell">'+label("1인 AR 축가")+'<h1 id="arHookTitle">떨리는 본식에서도<br><strong>내 목소리가 나를 받쳐줍니다</strong></h1><p>미리 녹음하고 자연스럽게 보정한 내 목소리를 AR로 함께 재생해, 현장에서는 마음에 집중해 노래합니다</p></div></section>'}
function arPrimaryBenefit(){return '<section class="shell section ar-primary-benefit" aria-labelledby="arPrimaryBenefitTitle"><span class="ar-intro-kicker">네, 맞습니다</span><h2 id="arPrimaryBenefitTitle">당신의 노래를 <strong>3배 이상 업그레이드해 줄</strong><br>요즘 유행하는 축가<br><em>“AR 사전 녹음”</em>입니다</h2></section>'}
function arOriginStory(){return '<section class="ar-origin-story section" aria-labelledby="arOriginTitle"><div class="shell"><span class="ar-origin-kicker">WISTIA · 제작 이야기</span><h2 id="arOriginTitle">7년 경력의 영상 편집 디자이너와<br>음악에 깊이 몰두해 온 엔지니어가 만났습니다</h2><p class="ar-origin-lead">한 곡의 노래가 한 편의 영상이 되기까지<br>목소리에 담긴 마음이 장면까지 이어지도록<br>녹음부터 음원 작업, 촬영과 편집을 함께합니다</p><figure class="ar-origin-photo">'+img("assets/img/song/solo.webp","녹음을 위해 준비된 마이크",true)+'</figure><div class="ar-origin-ending"><figure>'+img("assets/img/ar-process/02-mixing.webp","녹음한 목소리를 조율하는 장면",true)+'</figure><p><span class="ar-origin-star" aria-hidden="true">✱</span><strong>〈싱어게인2〉와 〈불후의 명곡〉 등<br>방송 음악 작업에 참여한 엔지니어의 손길과<br>7년간 쌓은 영상 편집 경험을 담았습니다</strong></p></div></div></section>'}
function prepareArHookVideo(){const video=document.querySelector(".ar-hook-media video"),button=document.querySelector("[data-ar-video-toggle]");if(!video||!button)return;const update=()=>{const playing=!video.paused&&!video.ended&&video.readyState>=2;video.classList.toggle("is-playing",playing);button.classList.toggle("is-playing",playing);button.querySelector("b").textContent=playing?"일시정지":"영상 재생";button.setAttribute("aria-label",playing?"실제 고객 본식 영상 일시정지":"실제 고객 본식 영상 재생")};video.addEventListener("playing",update);video.addEventListener("pause",update);video.addEventListener("ended",update);video.addEventListener("waiting",()=>video.classList.remove("is-playing"));video.play().catch(update);update()}
...
function startReviewCarousel(){cancelAnimationFrame(reviewFrame)}
...
function renderHome(requested="role"){
 if(requested==="role"){renderLanding();return}
...
function handleFinderChoice(el){
 if(el.name==="finderRole"){
  ...
  const next=el.value==="singer"?"#/song":el.value==="proposal"?"#/detail/proposal":"#/find/service"
...
function renderPicker(kind){
 const song=kind==="song";const keys=song?["solo","duo"]:["wedding","duet-film","solo-film"];
 app.innerHTML='<section class="shell section picker">'+label(song?"VOICE RECORDING":"WEDDING SONG FILM")+'<h1>'+(song?"현장에서는 편안하게<br>목소리는 미리 준비하세요":"우리 목소리를 담은<br>웨딩 영상")+'</h1><p class="lead">'+(song?"미리 녹음한 목소리가 함께 흐르는 AR로 본식의 긴장을 덜어보세요":"식전의 첫인사부터 축가 순서까지, 상영할 순간에 맞춰 골라보세요")+'</p><div class="picker-grid">'+keys.map((k,i)=>{const p=PRODUCTS[k],m=SERVICE_META[k],href="#/detail/"+k;return '<article>'+label(m.type)+(m.image?'<a class="picker-photo" href="'+href+'" aria-label="'+p.title+' 상품 알아보기">'+img(m.image,p.title+' 안내 이미지')+'</a>':'<div class="type-art" aria-hidden="true">'+(k==="duet-film"?"DUET":"SOLO")+'</div>')+'<h2>'+p.title+'</h2><p>'+m.short+'</p><dl><div><dt>노래하는 사람</dt><dd>'+m.who+'</dd></div><div><dt>받는 결과물</dt><dd>'+m.result+'</dd></div></dl>'+cta("상품 알아보기",href)+'</article>'}).join("")+'</div></section>'+footer()
}
...
function consultButton(key,extra=''){return '<button type="button" class="button kakao" data-consult-product="'+key+'" '+extra+'>카카오톡 상담 '+arrow()+'</button>'}
function bookingFacts(key,withCta=true){const p=PRODUCTS[key],film=FILM_FORMAT_PRODUCTS.has(key),format=FILM_FORMATS[selectedFilmFormat]?selectedFilmFormat:BASE_FILM_FORMAT[key],price=film?filmFormatPrice(key,format):p.normal;return '<section class="shell booking-facts" aria-label="가격과 기본 구성"><div class="facts-price"><div><small>기본가 · 이벤트 적용 전</small><strong>'+won(price)+'</strong></div><a class="text-link" href="#/event/'+key+(film?'/'+format:'')+'">이벤트 가격 확인 '+arrow()+'</a></div><dl><div><dt>기본 포함</dt><dd>'+(film?FILM_FORMATS[format].title+' · 녹음·디렉팅 · 보정·믹싱 · 영상 편집':'녹음·1:1 디렉팅 · 음정·박자 보정 · 믹싱·마스터링')+'</dd></div><div><dt>전달물</dt><dd>'+(film?'상영용 영상 + 완성 음원':'본식용 AR + 완성 음원')+'</dd></div><div><dt>제작 기간</dt><dd>'+(film?'촬영·자료 전달 후 약 14일':'녹음 완료 후 약 7일')+'<small>정확한 일정과 수정 범위는 상담에서 확인합니다.</small></dd></div></dl>'+(withCta?consultButton(key,'data-inline-consult'):'')+'</section>'}
function closingConsult(key){return '<section class="shell section closing-consult"><h2>아직 곡을 정하지<br>못했어도 괜찮아요</h2><p>사용 예정일과 참여 인원, 생각 중인 곡을 알려주세요. 필요한 준비부터 함께 안내해 드립니다.</p>'+consultButton(key,'data-inline-consult')+'<p class="fine">문의 내용을 복사한 뒤 카카오톡에서 직접 보내주세요. 상담만으로 예약이 확정되지는 않습니다.</p></section>'}
function compactProcess(p){const song=p.category==='song',steps=song?[
['곡과 일정 상담','사용일과 음역을 확인하고 곡·키와 AR 방향을 함께 정합니다.'],
['방문 녹음','마이크와 헤드폰을 맞춘 뒤 1:1 디렉팅으로 한 구간씩 녹음합니다.'],
['보정과 AR 제작','음정·박자·호흡을 다듬고 믹싱합니다. 리허설에 맞춰 목소리 비율을 함께 정합니다.'],
['확인 후 전달','완성 음원과 본식용 AR을 검수해 전달하고 사용 방법을 안내합니다.']]:[
['이야기와 구성 상담','상영 시점과 참여 인원, 원하는 분위기를 함께 정합니다.'],
['녹음과 촬영','구간별 디렉팅으로 노래를 녹음하고 선택한 구성에 맞춰 촬영합니다.'],
['음원과 영상 제작','목소리를 보정하고 믹싱한 뒤 장면·자막·색감을 편집합니다.'],
['확인 후 전달','선택한 구성과 수정 범위를 확인하고 상영용 영상과 완성 음원을 전달합니다.']];return '<section class="shell section compact-process" id="process">'+heading('','이렇게 준비합니다')+'<ol>'+steps.map((x,i)=>'<li><span>0'+(i+1)+'</span><div><h3>'+x[0]+'</h3><p>'+x[1]+'</p></div></li>').join('')+'</ol><details><summary>세부 제작 과정 보기</summary>'+processList(song?p.steps:filmProcessSteps(p.key,p.steps,selectedFilmFormat))+'</details><div class="craft-proof">'+img('assets/img/process/vocal-directing.webp','구간별 녹음과 보컬 디렉팅')+'<h3>목소리의 느낌은 그대로</h3><p>'+p.expertise.slice(0,2).map(x=>x[1]).join(' ')+'</p></div></section>'}
function usageComparison(){return '<section class="shell section usage-comparison">'+heading('','어떤 방식이 맞을까요?')+'<dl><div><dt>MR</dt><dd>반주에 맞춰 현장에서 직접 노래합니다.</dd></div><div><dt>AR</dt><dd>미리 녹음한 내 목소리가 포함된 음원과 함께 부릅니다.</dd></div><div><dt>영상 상영</dt><dd>현장에서 노래하는 대신 완성된 노래와 영상을 재생합니다. <a class="text-link" href="#/film">영상 상품 보기 →</a></dd></div></dl></section>'}
function renderDetail(key,purpose=''){
 const p=PRODUCTS[key],song=p.category==='song',film=FILM_FORMAT_PRODUCTS.has(key);
 if(film)selectedFilmFormat=FILM_FORMATS[purpose]&&FILM_FORMAT_PRICES[key]?.[purpose]?purpose:BASE_FILM_FORMAT[key];
 const w=WORKS.find(x=>x.product===key),title=key==='solo'?'내 목소리로 준비하는<br>조금 더 편안한 축가':p.title;
 const hero='<section class="shell product-intro">'+label(song?'VOICE RECORDING':'VOICE & FILM')+'<p class="product-name">'+p.title+'</p><h1>'+title+'</h1><p>'+p.sub+'</p></section>';
 const media=song?'<section class="shell section result-preview">'+heading('','본식에서는 이렇게 함께 부릅니다')+'<video controls playsinline preload="none" poster="'+(key==='solo'?'assets/img/ar-detail/solo-live-proof.jpg':SERVICE_META[key].image)+'" aria-label="'+p.title+' 실제 본식 영상"><source src="'+p.resultVideo+'" type="video/mp4"></video><p class="fine">실제 고객님이 보내주신 본식 영상입니다.</p><p class="media-error" hidden>영상을 불러오지 못했습니다. 네트워크 연결을 확인한 뒤 다시 재생해 주세요.</p></section>':
 '<section class="shell section result-preview">'+heading('','목소리가 한 편의 영상으로')+(w?'<button class="film-poster" data-video="'+w.id+'" aria-label="'+p.title+' 실제 영상 재생">'+img(w.image,w.title,true)+'<span class="play" aria-hidden="true">▶</span></button><p class="fine">'+(key==='proposal'?'실제 프로포즈 영상입니다.':'뮤직 비디오 필름 예시입니다. 기본 메이킹 구성과 포함 장면이 다릅니다.')+'</p>':compactFilmHeroMedia(key)+'<p class="fine">녹음 메이킹 필름 구성 예시입니다.</p>')+'</section>';
 app.innerHTML=hero+detailPersonTabs(key)+bookingFacts(key)+(song?arRatio():'')+media+reviews()+(!song?'<section class="shell section film-scope">'+heading('','선택한 영상 구성')+'<h3>'+FILM_FORMATS[selectedFilmFormat].title+'</h3><p>'+filmFormatSummary(key,selectedFilmFormat)+'</p>'+filmFormatNotes(FILM_FORMATS[selectedFilmFormat])+'</section>'+filmUpgradeDetail(key):'')+compactProcess(p)+(song?usageComparison()+detailNext(key):'')+'<section class="shell section worry-section">'+heading('','자주 묻는 질문')+faq(p.faq)+'</section>'+closingConsult(key)+footer()+priceBar(key);
}
...
function arRatio(){return '<section class="shell section ratio-section">'+heading('','내 목소리가 얼마나 들릴까요?','같은 노래로 목소리 비율을 비교해 보세요')+'<div class="ratio-buttons" role="group" aria-label="AR 목소리 비율">'+['30','50','70','100'].map(n=>'<button data-ratio="'+n+'" aria-pressed="'+(n===voiceRatio)+'">'+n+'%</button>').join('')+'</div><p class="ratio-current">현재 선택 <strong id="ratioValue">'+voiceRatio+'%</strong></p><audio id="ratioAudio" controls preload="metadata" aria-label="AR 비교 음원"></audio><p id="ratioHelp" role="status">선택한 음원을 준비하고 있습니다.</p><button class="text-link" data-ratio-retry hidden>음원 다시 불러오기</button><p>실제 목소리 비율은 스튜디오 리허설에서 함께 결정합니다.</p><p class="fine">샘플곡 · 그중에 그대를 만나</p></section>'}
...
async function prepareRatioAudioSources(){const audio=document.querySelector('#ratioAudio');if(!audio)return;audio.addEventListener('loadedmetadata',()=>{document.querySelector('#ratioHelp').textContent='재생 중 비율을 바꾸면 같은 위치에서 비교할 수 있습니다.';document.querySelector('[data-ratio-retry]').hidden=true});audio.addEventListener('error',()=>{delete audio.dataset.switching;document.querySelector('#ratioHelp').textContent='이 음원을 불러오지 못했습니다. 다른 비율을 선택하거나 다시 시도해 주세요.';document.querySelector('[data-ratio-retry]').hidden=false});audio.src=ratioSource(voiceRatio);audio.dataset.ratioAudio=voiceRatio;audio.load()}
...
async function switchRatioAudio(ratio){const audio=document.querySelector('#ratioAudio');if(!audio)return;const token=++ratioSwitchToken,position=audio.currentTime||0,playing=audio.dataset.switching==='true'?audio.dataset.resume==='true':!audio.paused;audio.dataset.switching='true';audio.dataset.resume=String(playing);audio.pause();audio.src=ratioSource(ratio);audio.dataset.ratioAudio=ratio;document.querySelector('#ratioHelp').textContent='선택한 음원을 준비하고 있습니다.';audio.addEventListener('loadedmetadata',()=>{if(token!==ratioSwitchToken)return;delete audio.dataset.switching;audio.currentTime=Math.min(position,Number.isFinite(audio.duration)?Math.max(0,audio.duration-.05):position);if(playing)audio.play().catch(()=>{})},{once:true});audio.load()}
...
function renderArPurpose(key){renderDetail('solo');const intro=document.querySelector('.product-intro>p:last-child');if(intro)intro.textContent=AR_PURPOSES[key].lead}
...
function calculate(){
 ...
 const songOption=product.category==="song"?SONG_OPTIONS.find(o=>o.key===chosenOption):null;
  ...
  const hasUnquoted=Boolean((songOption&&songOption.price==null)||optionEntries.some(o=>o.price==null&&!o.notice));
  return {product,chosen,discount,optionEntries,songOption,formatUpgrade,optionPrice,hasUnquoted,finalPrice:product.normal+optionPrice-discount}
}
...
function renderEvent(key,purpose=""){
 ...
 if(["solo","duo"].includes(key)&&purpose==="lyrics")chosenOption="lyrics";
  currentEventProduct=key;currentEventPurpose=purpose;if(FILM_FORMAT_PRODUCTS.has(key))selectedFilmFormat=FILM_FORMATS[purpose]&&FILM_FORMAT_PRICES[key]?.[purpose]?purpose:BASE_FILM_FORMAT[key];const p=PRODUCTS[key];
 ...
 app.innerHTML='<section class="shell section booking-calculator-wrap"><form id="consultForm" class="booking-calculator"><header class="calculator-intro"><a class="text-link" href="'+(film?'#/detail/'+key+'/'+BASE_FILM_FORMAT[key]:'#/detail/'+key)+'">'+p.title+' 상품 안내 '+arrow()+'</a><p class="eyebrow">PRICE CALCULATOR</p><h1>가격 계산기</h1></header>'+bookingFacts(key,false)+selection+choices+'<div id="bookingSummary" aria-live="polite"></div>'+consultationFields()+'<div class="booking-mobile-bar booking-static-bar"><span id="mobilePrice"></span><button class="button kakao" type="submit">문의 내용 확인 · 카카오톡 상담 '+arrow()+'</button></div></form></section>'+footer()
 updatePrice()
}
function updatePrice(){let c=calculate();const facts=document.querySelector(".booking-calculator .booking-facts");if(facts)facts.outerHTML=bookingFacts(currentEventProduct,false);const formatRow=FILM_FORMAT_PRODUCTS.has(currentEventProduct)?'<div><dt>영상 구성</dt><dd>'+FILM_FORMATS[selectedFilmFormat].title+'</dd></div>':'';const finder=finderPriceContext(currentEventProduct),finderRow=finder?'<div class="finder-summary-line"><dt>선택한 조건</dt><dd>'+FINDER_LABELS.role[finder.role]+' · '+FINDER_LABELS.moment[finder.moment]+' · '+FINDER_LABELS.people[finder.people]+'</dd></div>':'';const formatOption=c.formatUpgrade?'<div><dt>뮤직 비디오 필름 업그레이드</dt><dd class="plus">'+(c.formatUpgrade>0?'+':'−')+won(Math.abs(c.formatUpgrade))+'</dd></div>':'',optionRows=formatOption+(c.songOption?'<div><dt>'+c.songOption.label+'</dt><dd class="plus">'+(c.songOption.price?'+'+won(c.songOption.price):'상담 후 안내')+'</dd></div>':'')+c.optionEntries.map(o=>'<div><dt>'+o.label+(o.quantity>1?' '+o.quantity+'회':'')+'</dt><dd class="plus">'+(o.price==null?'별도 견적':'+'+won(o.total))+'</dd></div>').join("");const eventRows=c.chosen.map(e=>'<div><dt>'+e.label+'</dt><dd class="minus">−'+won(e.discount)+'</dd></div>').join("");document.querySelector("#bookingSummary").innerHTML='<p class="eyebrow"><span class="desktop-summary-label">가격 계산</span><span class="mobile-summary-label">선택한 구성</span></p><h2>'+c.product.title+'</h2><dl class="price-lines">'+finderRow+formatRow+'<div class="normal-line"><dt>기본가</dt><dd>'+won(c.product.normal)+'</dd></div>'+optionRows+eventRows+'</dl><div class="price-formula"><span><small>기본가</small>'+won(c.product.normal)+'</span><i>+</i><span><small>추가 옵션</small>'+won(c.optionPrice)+'</span><i>−</i><span><small>이벤트 혜택</small>'+won(c.discount)+'</span><i>=</i><strong><small>'+(c.hasUnquoted?'확인 가능한 소계':'예상 금액')+'</small>'+won(c.finalPrice)+'</strong></div><p class="fine">선택한 조건을 기준으로 계산한 예상 금액이며 최종 적용 여부는 상담에서 확인합니다</p>';const net=c.optionPrice-c.discount,normalPrice=c.discount>0?'<del>'+shortWon(c.product.normal)+'</del>':'<small class="normal-price">'+shortWon(c.product.normal)+'</small>';document.querySelector("#mobilePrice").innerHTML='<span><em>기본가</em>'+normalPrice+'</span><span><em>옵션·혜택</em><small class="'+(net>0?'plus':'minus')+'">'+(net>0?'+':net<0?'−':'')+shortWon(Math.abs(net))+'</small></span><span><em>'+(c.hasUnquoted?'확인 가능한 소계':'예상 금액')+'</em><strong>'+shortWon(c.finalPrice)+'</strong></span>'+(c.hasUnquoted?'<p class="quote-note">선택한 별도 견적 옵션의 금액은 포함되지 않았습니다.</p>':'')}
function consultationText(){let c=calculate(),picked=c.optionEntries.map(o=>o.label+(o.price==null?' · 별도 견적':' +'+won(o.total)));if(c.formatUpgrade)picked.unshift('뮤직 비디오 필름 업그레이드 '+(c.formatUpgrade>0?'+':'−')+won(Math.abs(c.formatUpgrade)));if(c.songOption)picked.unshift(c.songOption.label+(c.songOption.price?' +'+won(c.songOption.price):' · 상담 후 안내'));return ["🤍 🇼 🇪 🇱 🇨 🇴 🇲 🇪  🤍","","아래 문의 양식을 복사한 후","내용을 작성해 보내주세요 :D","","보내주신 내용을 확인한 후","최대한 빠르게 안내드리겠습니다 :)","━━━━━","","[ 𝐂𝐨𝐧𝐭𝐚𝐜𝐭 𝐅𝐨𝐫𝐦 ]","","• 어디에서 보고 오셨나요? : "+(consultationDraft.source||""),"• 성함 : "+(consultationDraft.name||""),"• 희망 서비스 : "+c.product.title,FILM_FORMAT_PRODUCTS.has(currentEventProduct)?"• 선택한 영상 구성 : "+FILM_FORMATS[selectedFilmFormat].title:null,"• 예식일 또는 사용 예정일 : "+(consultationDraft.eventDate||""),"• 희망 예약일 : 상담 시 협의","• 희망 시간 : 상담 시 협의","• 상영 시점 : "+(consultationDraft.purpose||""),"","• 선택한 추가 옵션 : "+(picked.length?picked.join(" / "):"선택 없음"),"• 이벤트 혜택 : "+(c.chosen.length?c.chosen.map(e=>e.label+" −"+won(e.discount)).join(" / "):"선택 없음"),(c.hasUnquoted?"• 확인 가능한 소계 (별도 견적 옵션 제외) : ":"• 예상 금액 : ")+won(c.finalPrice)].filter(x=>x!==null).join("\n")}
...
function consultationDialog(text){showDialog('<h2>카카오톡으로 이어서 상담해요</h2><p>아래 내용을 복사한 뒤 카카오톡 채팅창에 붙여넣어 직접 보내주세요.</p><textarea id="consultationCopy" rows="9" readonly aria-label="상담 문의 내용">'+escapeHtml(text)+'</textarea><button class="button kakao" data-copy-consult>문의 내용 복사 후 카카오톡 열기</button><p id="copyStatus" role="status"></p>'+external('카카오톡 직접 열기',kakao()),'consult-dialog')}
async function copyConsultationAndOpenKakao(){consultationDialog(consultationText())}
async function copyConsultation(){const area=document.querySelector('#consultationCopy');let ok=false;try{await navigator.clipboard.writeText(area.value);ok=true}catch{area.select();try{ok=document.execCommand('copy')}catch{}}const status=document.querySelector('#copyStatus');status.textContent=ok?'문의 내용이 복사되었습니다. 카카오톡에 붙여넣어 주세요.':'자동 복사가 제한되었습니다. 위 내용을 직접 복사하고 아래 카카오톡 링크를 눌러주세요.';if(ok){const popup=window.open(kakao(),'_blank');if(popup)popup.opener=null;else status.textContent+=' 아래 카카오톡 직접 열기를 눌러주세요.'}}
...
function enhanceMotion(){routeObserver?.disconnect()}
let firstRender=true
function route(){
 closeDialog();ratioSwitchToken++;routeObserver?.disconnect();const parts=(location.hash.replace(/^#/,"")||"/").split("/").filter(Boolean);const [type,key]=parts;
...
document.addEventListener("click",e=>{
 ...
 if(el.hasAttribute('data-review-open')){const opener=lastDialogFocus;const wasOpen=dialog.open;openReview(Number(el.dataset.reviewOpen));if(wasOpen)lastDialogFocus=opener;return}
 if(el.hasAttribute('data-copy-consult')){copyConsultation();return}
 if(el.hasAttribute('data-consult-product')){const key=el.dataset.consultProduct;consultationDialog('안녕하세요. '+PRODUCTS[key].title+' 상담을 원합니다.\n'+(FILM_FORMAT_PRODUCTS.has(key)?'영상 구성: '+FILM_FORMATS[selectedFilmFormat].title+'\n':'')+'사용 예정일: \n참여 인원: \n생각 중인 곡: ');return}
 if(el.hasAttribute('data-ratio-retry')){switchRatioAudio(voiceRatio);return}
...
document.addEventListener("change",e=>{
 ...
 if(el.dataset.songOption){chosenOption=el.checked&&el.dataset.songOption!=="none"?el.dataset.songOption:"";updatePrice()}
...
function renderLanding(){app.innerHTML='<section class="shell landing-intro"><p class="eyebrow">WISTIA · VOICE & FILM STUDIO</p><h1>우리의 목소리로<br>남기는 순간</h1><p>축가 녹음부터 웨딩·프로포즈 영상까지.<br>처음 녹음하는 순간부터 함께 준비합니다.</p>'+img('assets/img/hero/wistia-hero-main.webp','위스티아 웨딩 보이스·필름 스튜디오',true)+'</section><section class="shell section landing-services" id="services"><h2>어떤 순간을 준비하시나요?</h2>'+[['#/song','직접 부르는 축가','내 목소리를 미리 녹음한 AR과 함께 노래해요','assets/img/song/solo.webp'],['#/film','우리 노래를 담은 영상','식전 또는 축가 순서에 완성된 영상을 보여줘요','assets/img/wedding/03-lipsync-mv.webp'],['#/detail/proposal','프로포즈 · 답프로포즈','노래와 영상으로 고백을 전해요','assets/img/proposal-video-cover.jpg']].map(x=>'<a class="service-choice" href="'+x[0]+'">'+img(x[3],x[1])+'<span><strong>'+x[1]+'</strong><small>'+x[2]+'</small></span><b aria-hidden="true">→</b></a>').join('')+'<details class="service-helper"><summary>어떤 서비스가 맞을지 모르겠어요</summary><p>현장에서 직접 노래할 예정인가요?</p><a class="text-link" href="#/song">네, 직접 부를 거예요 →</a><p>노래 대신 영상을 보여주고 싶다면</p><a class="text-link" href="#/find/service" data-start-film-finder>상영 시점과 인원으로 찾기 →</a></details><details><summary>전체 서비스 보기</summary>'+serviceRows(SERVICE_ORDER)+'</details></section>'+footer()}
let consultObserver;
function enhanceUiux(){consultObserver?.disconnect();const floating=document.querySelector('#floatingKakao');floating.hidden=true;const bar=document.querySelector('.consult-bar');if(bar){const targets=[...document.querySelectorAll('[data-inline-consult],.closing-consult,.footer')];const update=()=>{const visible=targets.some(n=>{const r=n.getBoundingClientRect();return r.top<innerHeight&&r.bottom>66});const first=document.querySelector('.booking-facts');bar.hidden=visible||!first||first.getBoundingClientRect().top>innerHeight};consultObserver=new IntersectionObserver(update,{threshold:[0,1]});targets.forEach(n=>consultObserver.observe(n));if(document.querySelector('.booking-facts'))consultObserver.observe(document.querySelector('.booking-facts'));update()}
 document.querySelectorAll('video,audio').forEach(media=>{media.addEventListener('play',()=>{document.querySelectorAll('video,audio').forEach(other=>{if(other!==media)other.pause()})});if(media.tagName==='VIDEO')media.addEventListener('error',()=>{const note=media.parentElement.querySelector('.media-error');if(note)note.hidden=false})});
}
document.addEventListener('click',event=>{if(event.target.closest('[data-start-film-finder]')){finderChoice.role='couple';renderHome('service')}});

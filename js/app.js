...
function initSoloReviewShowcase(){const root=document.querySelector('[data-review-showcase]');if(!root)return;const slides=[...root.querySelectorAll('[data-review-slide]')],status=root.querySelector('[data-review-status]'),next=root.querySelector('[data-solo-review-next]');let index=0;const render=()=>{slides.forEach((slide,i)=>{const active=i===index;slide.classList.toggle('is-active',active);slide.setAttribute('aria-hidden',String(!active))});if(status)status.textContent=String(index+1).padStart(2,'0')};next?.addEventListener('click',()=>{index=(index+1)%slides.length;render()})}
...
const AR_COMPARE_ROWS=[["준비 범위","단계별 개별 진행","상담부터 AR 제작까지 한 번에"],["녹음 방식","완곡 중심의 일괄 녹음","구간별 1:1 디렉팅"],["보컬 보정","기본 음정·박자 보정","음색을 살린 수작업 보정"],["최종 전달","완성 음원 제공","완성 음원 + 본식용 AR 제공"]]
function arComparisonSection(){return '<section class="ar-compare section" aria-labelledby="arCompareTitle" data-ar-compare><div class="shell"><header class="ar-compare-intro" data-solo-section-intro><p data-solo-kicker>WHY WISTIA</p><h2 id="arCompareTitle" class="ar-compare-title" data-solo-title><span class="solo-type-line">같은 녹음이라도,</span><span class="solo-type-line">결과는 같지 않습니다.</span></h2><p class="ar-compare-sub" data-solo-sub><span class="solo-type-line">비슷해 보이는 서비스도 완성도를 만드는 기준은 다릅니다.</span><span class="solo-type-line">일반 제작과 위스티아의 차이를 직접 비교해보세요.</span></p></header><div class="ar-compare-table" role="table" aria-label="일반 제작과 WISTIA 비교"><div class="ar-compare-head" role="row"><span role="columnheader">일반 제작</span><strong role="columnheader">WISTIA</strong></div>'+AR_COMPARE_ROWS.map((row,i)=>'<div class="ar-compare-row" data-ar-compare-row role="row" style="--row-i:'+i+'"><span class="ar-compare-general" role="cell">'+row[1]+'</span><strong class="ar-compare-criteria" role="rowheader">'+row[0]+'</strong><span class="ar-compare-wistia" role="cell">'+row[2]+'</span></div>').join("")+'</div><p class="ar-compare-note">제공 범위는 업체와 상품에 따라 달라질 수 있습니다.</p></div></section>'}
...
function soloProcessSlider(){const first=SOLO_STUDIO_PROCESS[0];return '<section class="wistia-process-studio solo-process-section" id="process" data-process-studio aria-labelledby="processStudioTitle">'
 +'<header class="wps-head" data-solo-section-intro><div><p class="wps-kicker" data-solo-kicker>WISTIA · PRODUCTION PROCESS</p><h2 id="processStudioTitle" data-solo-title><span class="solo-type-line">처음부터 끝까지,</span><span class="solo-type-line">맞춤형으로 케어해드립니다.</span></h2><p class="wps-sub" data-solo-sub><span class="solo-type-line">상담부터 녹음과 보정, 본식용 AR 전달까지</span><span class="solo-type-line">일곱 단계를 순서대로 확인해 보세요.</span></p></div><div class="wps-counter" aria-label="현재 프로세스 01 / 07"><span data-process-current>01</span><i></i><span>07</span></div></header>'
 ...
 +'<nav class="wps-chapters" aria-label="제작 단계 선택" hidden>'+SOLO_STUDIO_PROCESS.map((step,index)=>'<button type="button" class="wps-chapter'+(index===0?' is-active':'')+'" data-process-chapter="'+index+'" data-image="'+step.image+'" aria-current="'+(index===0?'step':'false')+'" aria-label="'+String(step.id).padStart(2,"0")+' '+escapeHtml(step.short)+' 단계 보기"><b>'+String(step.id).padStart(2,"0")+'</b><span>'+escapeHtml(step.short)+'</span></button>').join("")+'</nav></section>'}
...
function renderHome(requested="role"){
 ...
 const choices=(name,items)=>items.map(([value,title,example,kind])=>'<label class="finder-choice"><input type="radio" name="'+name+'" value="'+value+'"><span class="finder-choice-mark" aria-hidden="true"></span><span class="finder-choice-content">'+(kind?'<span class="finder-service-badge">'+serviceIcon(kind)+'<em>'+kind+'</em></span>':'')+'<strong>'+title+'</strong>'+(example?'<small>'+example+'</small>':'')+'</span></label>').join("")
 ...
 app.innerHTML='<section class="finder-home shell finder-stage-'+stage+(showIntro?' has-intro-animation':'')+'" aria-labelledby="finderTitle">'+splash+'<div class="finder-content">'+(stage==="role"?"":'<a class="finder-back" href="'+back+'">← 이전 질문</a>')+'<header class="finder-intro">'+(stage==="role"?"":'<span class="eyebrow">'+config.number+'</span>')+'<h1 id="finderTitle">'+config.title+'</h1><p>'+config.description+'</p></header><div class="finder-steps"><fieldset class="finder-step"><legend class="sr-only">'+config.title+'</legend><div class="finder-options">'+choices(config.name,config.items)+'</div></fieldset></div></div></section>'
 updateHomeFinder()
...
function initSoloProcessSlider(){
 ...
 const schedule=()=>{clearTimeout(autoTimer);if(paused||!root.isConnected)return;autoTimer=setTimeout(()=>{render((activeIndex+1)%SOLO_STUDIO_PROCESS.length);schedule()},3000)}

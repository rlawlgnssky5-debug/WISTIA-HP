...
function arHookHero(p){return '<section class="ar-hook-hero" aria-labelledby="arHookTitle"><div class="ar-hook-media">'+img("assets/img/ar-detail/solo-live-proof.jpg","실제 고객의 본식 AR 축가 현장",true).replace('<img ','<img class="ar-hook-fallback" ')+'<video autoplay muted loop playsinline preload="auto" poster="assets/img/ar-detail/solo-live-proof.jpg" aria-label="실제 고객의 본식 AR 축가 현장 영상"><source src="'+escapeHtml(p.resultVideo)+'" type="video/mp4">영상을 재생할 수 없는 브라우저입니다</video><button type="button" class="ar-hook-video-control" data-ar-video-toggle aria-label="실제 고객 본식 영상 재생"><span aria-hidden="true">▶</span><b>영상 재생</b></button></div><div class="ar-hook-copy shell">'+label("1인 AR 축가")+'<h1 id="arHookTitle">떨리는 본식에서도<br><strong>내 목소리가 나를 받쳐줍니다</strong></h1><p>미리 녹음하고 자연스럽게 보정한 내 목소리를 AR로 함께 재생해, 현장에서는 마음에 집중해 노래합니다</p></div></section>'}
function arPrimaryBenefit(){return '<section class="shell section ar-primary-benefit" aria-labelledby="arPrimaryBenefitTitle"><div class="ar-primary-heading">'+label("가장 큰 장점")+'<h2 id="arPrimaryBenefitTitle">노래를 한 번에 완벽하게 부르는 대신,<br><strong>진심을 끝까지 전할 수 있도록</strong></h2></div><figure class="ar-primary-image">'+img("assets/img/ar-detail/solo-live-proof.jpg","실제 고객의 본식 AR 축가 현장",true)+'<figcaption>실제 고객 AR 축가 현장</figcaption></figure><div class="ar-primary-copy"><p>스튜디오에서 한 소절씩 녹음하고 음정과 박자를 자연스럽게 다듬은 뒤, 본식에서 편하게 부를 수 있는 목소리 비율로 AR을 완성합니다</p><ol aria-label="AR 축가 이용 흐름"><li><span>01</span><strong>내 목소리 녹음·보정</strong></li><li><span>02</span><strong>본식에 맞는 비율 선택</strong></li><li><span>03</span><strong>현장에서 함께 노래</strong></li></ol><div class="ar-ratio-point"><strong>30 · 50 · 70 · 100%</strong><span>본식에 맞춰 선택하는 AR 속 내 목소리 비율</span></div></div></section>'}
...
function detailComparison(p){const film=p.category!=="song",rows=film?[["준비 방식","녹음·촬영·편집을 각각 확인","한 곳에서 녹음부터 상영본까지"],["녹음 진행","연습 후 전체 곡 녹음","구간별 1:1 보컬 디렉팅"],["보컬 보정","보정 범위와 비용을 별도 확인","음정·박자·호흡까지 수작업 보정"],["최종 전달","제공 파일을 항목별로 확인","완성 음원과 상영용 영상 함께 전달"]]:[["준비 방식","반주·녹음·보정을 각각 확인","녹음부터 본식용 AR까지 한 번에"],["녹음 진행","연습 후 전체 곡 녹음","구간별 1:1 보컬 디렉팅"],["보컬 보정","보정 범위와 비용을 별도 확인","원래 목소리를 살린 수작업 보정"],["최종 전달","최종 음원 종류를 별도 확인","완성 음원과 본식용 AR 함께 전달"]];return '<section class="comparison-band section" aria-labelledby="comparisonTitle"><div class="shell">'+heading("","일반 제작 방식과 비교해 보세요","예약 전에 반드시 확인해야 할 제작 범위를 기준으로 정리했습니다")+'<div class="comparison-table" role="table" aria-label="일반 제작 방식과 위스티아 비교"><div class="comparison-head" role="row"><span></span><strong>일반 제작 방식</strong><strong>WISTIA</strong></div>'+rows.map(row=>'<div class="comparison-row" role="row"><strong>'+row[0]+'</strong><span><small>일반 제작 방식</small>'+row[1]+'</span><span><small>WISTIA</small>'+row[2]+'</span></div>').join("")+'</div><p class="comparison-note">업체별 상품 구성은 다를 수 있으므로 실제 포함 범위와 추가 비용은 예약 전에 확인해 주세요</p></div></section>'}
...
function renderDetail(key,purpose=""){
 ...
 if(key==="solo"){
  app.innerHTML=detailPersonTabs(key)+arHookHero(p)+arPrimaryBenefit()+reviews(key)+detailNext(key)+process+arRatio()+detailComparison(p)+detailIncluded(p)+'<section class="shell section worry-section">'+heading("","자주 묻는 질문")+faq(p.faq)+'</section>'+footer()+priceBar(key)
  return
 }

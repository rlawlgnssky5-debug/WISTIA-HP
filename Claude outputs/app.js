...
function renderDetail(key,purpose=""){
 ...
 if(key==="solo"){
  app.innerHTML=detailPersonTabs(key)+'<div class="solo-detail-scope">'+arHookHero(p)+arPrimaryBenefit()+reviews(key)+arExpertStory()+process+arRatio()+detailComparison(p)+'<section class="shell section worry-section">'+heading("","자주 묻는 질문")+faq(p.faq)+'<div class="detail-faq-price">'+cta("이벤트 적용 가격 확인하기","#/event/solo")+'</div></section>'+footer()+priceBar(key)+'</div>'
  return
 }

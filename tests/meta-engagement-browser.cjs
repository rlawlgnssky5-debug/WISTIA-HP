...
;(async () => {
  ...
  try {
    for (const width of [320, 390]) {
      ...
      await page.locator('#mediaDialog .consult-copy-action').waitFor()
      assert.ok((await page.locator('#mediaDialog .consult-copy-action').getAttribute('href')).includes('pf.kakao.com'))
      assert.equal(await page.locator('#floatingKakaoChat').count(), 1)
      assert.ok(await page.locator('.footer a[href*="pf.kakao.com"]').count() > 0)

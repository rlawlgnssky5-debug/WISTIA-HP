...
assert.match(design, /\.ar-compare-table\{[^}]*width:min\(88%/)
assert.doesNotMatch(app, /role="columnheader">비교 항목/)
assert.match(design, /grid-template-columns:30fr 15fr 55fr/)
assert.match(design, /\.ar-compare-table\{[^}]*margin-left:auto;[^}]*margin-right:0/)
assert.doesNotMatch(design, /\.ar-compare-head>strong\{[^}]*top:-8px/)
assert.match(design, /\.ar-compare-row:last-child \.ar-compare-criteria\{border-radius:0!important/)
assert.match(design, /@keyframes solo-wistia-emphasis/)
assert.match(design, /\.ar-copy-emphasis-bg\{[^}]*inset-inline:/)
assert.match(design, /wistia-waveform\.png/)
assert.match(ratio, /grid-template-columns:\s*510px 450px 82px/)

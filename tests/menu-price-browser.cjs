...
;(async () => {
  ...
  try {
    for (const width of [320, 390, 600]) {
      ...
      assert.equal(await page.locator('#siteHeader').evaluate(node => getComputedStyle(node).position), 'fixed')
      assert.equal(await page.locator('#siteHeader').evaluate(node => Math.round(node.getBoundingClientRect().top)), 0)
      assert.equal(await page.locator('body').evaluate(node => Math.round(parseFloat(getComputedStyle(node).paddingTop))), 48)
      ...
      assert.equal(await page.evaluate(() => Boolean(document.elementFromPoint(innerWidth / 2, 12)?.closest('#siteHeader'))), true)
      await page.locator('#menuToggle').click()
  ...
  console.log('Solid fixed menu header and price notice checks passed at 320px/390px/600px')
